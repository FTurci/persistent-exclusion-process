...
def main():
    ...
    if not os.path.isfile("../data/no-rolling/sampler_records.csv"):
        os.system("echo ',tumble,n_iter,density,speed' > ../data/no-rolling/sampler_records.csv")
    df.to_csv("../data/no-rolling/sampler_records.csv", mode="a", header=False)
    ...
    for idx, tumble in tqdm.tqdm(enumerate(tumbles)):
        ...
        with h5py.File(
            f"../data/no-rolling/dataset_tumble_{tumble:.3f}_{args.density}.h5", "w"
        ) as f_out:
